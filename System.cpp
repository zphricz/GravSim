...
System::System() : minimum_distance(0.0) {}
...
void System::calculate_forces(double time_step) {
  for (int i = 0; i < num_bodies() - 1; ++i) {
    for (int j = i + 1; j < num_bodies(); ++j) {
      ...
      double force_mag =
          UGC * bodies[i].mass * bodies[j].mass / delta_pos.magnitude_square();
  ...
  for (int i = 0; i < num_bodies(); ++i) {
    for (int j = i + 1; j < num_bodies(); ++j) {
      ...
      if (delta_pos.magnitude() < minimum_distance) {
        Vec2d momentum =
            bodies[j].vel * bodies[j].mass + bodies[i].vel * bodies[i].mass;
...
void System::add_body(Body &body) { bodies.push_back(body); }

void System::add_body(Body &&body) { bodies.push_back(body); }

void System::add_body(double x_pos, double y_pos, double x_vel, double y_vel,
                      double mass, int track_length, bool movable) {
  add_body(Body(x_pos, y_pos, x_vel, y_vel, mass, track_length, movable));
}

int System::num_bodies() { return bodies.size(); }
